...
import { GUARDIAN_ARTICLES, FREE_GUARDIAN_RESOURCES_COUNT } from '../../config/studentContent';
import { getBadgeClassName } from '../../utils/helpers';

interface GuardianResourcesProps {
  isPublic?: boolean;
}

export interface GuardianResourcesRef {
  openMobileMenu: () => void;
}

/**
 * GuardianResources Component
 ...
 * Public mode: Locks articles after FREE_GUARDIAN_RESOURCES_COUNT
...
const GuardianResources = forwardRef<GuardianResourcesRef, GuardianResourcesProps>(({ isPublic = false }, ref) => {
  ...
  const isArticleLocked = (index: number): boolean => {
    return isPublic && index >= FREE_GUARDIAN_RESOURCES_COUNT;
  };
      ...
      {isListOpen && (
        <div className="hidden lg:flex w-80 flex-col border-r border-gray-200 bg-white h-full shrink-0">
          <div className="p-4 border-b border-gray-200 flex items-center justify-between">
            <h2 className="font-bold text-gray-900">Parenting Resources</h2>
            <button
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
            {GUARDIAN_ARTICLES.map((article, index) => {
...
GuardianResources.displayName = 'GuardianResources';

export default GuardianResources;
