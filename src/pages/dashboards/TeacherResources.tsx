...
export interface TeacherResourcesRef {
  openMobileMenu: () => void;
}

/**
 * TeacherResources Component
 ...
 * Public mode: Locks articles after FREE_TEACHER_RESOURCES_COUNT
...
const TeacherResources = forwardRef<TeacherResourcesRef, TeacherResourcesProps>(({ isPublic = false }, ref) => {
  ...
  const isArticleLocked = (index: number): boolean => {
    return isPublic && index >= FREE_TEACHER_RESOURCES_COUNT;
  };
      ...
      {isListOpen && (
        <div className="hidden lg:flex w-80 flex-col border-r border-gray-200 bg-white h-full shrink-0">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
            {TEACHER_ARTICLES.map((article, index) => {
...
TeacherResources.displayName = 'TeacherResources';
